...
def ft_stream_management() -> None:
    ...
    satus_report = stdin.readline().strip()
    print()
    msg_sterr = "Communication channels verified"
    print(f"[STANDER] Archive status from {(archivist_id)}: " +
          f"{satus_report}", file=stdout)
    print(f"[ALERT] System diagnostic: {str(msg_sterr)}", file=stderr)
